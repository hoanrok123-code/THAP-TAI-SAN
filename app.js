...
// Định nghĩa 5 tầng tháp tài sản chuẩn cùng tỷ lệ khuyến nghị (%)
const LAYERS_CONFIG = [
...
// Xử lý định dạng tiền tệ và số liệu
...
// Hàm hỗ trợ nhập liệu tiền tệ có phân cách dấu chấm "." và số thập phân ","
function formatNumberInput(val) {
  if (val === undefined || val === null || val === "") return "";
  let str = val.toString().replace(/\./g, ""); // Xóa dấu phân cách hàng nghìn
  let parts = str.split(",");
...
function bindMoneyInput(inputEl) {
  inputEl.addEventListener("input", function(e) {
    ...
    let raw = e.target.value.replace(/[^0-9,]/g, ""); // Chỉ cho phép số và dấu phẩy
...
// Logic tính toán dòng tiền & chỉ số tài chính
...
function totals(){
  ...
  const totalDebtPaymentMonthly = totalLoanInterest + totalPrincipal; // Tổng trả nợ NH/tháng
  
  const totalIncomeMonthly = +S.income + passive; // Tổng dòng tiền thu
  const totalLivingExpense = +S.expense; // Chi tiêu sinh hoạt
  const totalExpenseMonthly = totalLivingExpense + totalDebtPaymentMonthly; // Tổng chi ra
  
  const surplus = totalIncomeMonthly - totalExpenseMonthly;
  
  // Các chỉ số đánh giá sức khỏe tài chính
...
function goalPlan(g){
  ...
  // Cần bổ sung thêm bao nhiêu tiền/tháng nếu bị thiếu
...
function dashboard(){
  ...
  // Đánh giá tỷ lệ Nợ / Dòng tiền thu
  ...
  // Đánh giá tỷ lệ Tổng chi / Dòng tiền thu
  ...
  document.getElementById("app").innerHTML = `
    ...
    <div class="card">
      ...
      <p class="small muted" style="margin-top:10px">Hạn: ${datef(g.deadline)} • Vốn gán sẵn: ${money(p.initialCap)} (Gồm tài sản chọn + nguồn thu xếp sau)</p>
...
// GIAO DIỆN THÁP TÀI SẢN (5 MỤC NHẬP TÁCH BIỆT, KHÔNG GIỚI HẠN)
function tower(){
...
// FORM THÊM / SỬA TÀI SẢN (HỖ TRỢ VÀNG & ĐỊNH DẠNG SỐ DẤU CHẤM)
function assetForm(id = null, defaultLayer = 1){
  ...
  // Bind tự động dấu phân cách "." khi gõ số
...
// FORM CẤU HÌNH DÒNG TIỀN & THU NHẬP
function settings(){
  document.getElementById("app").innerHTML = `
    <div class="card">
...
// GIAO DIỆN MỤC TIÊU TÀI CHÍNH (GÁN TÀI SẢN KHÔNG GIỚI HẠN)
function goals(){
...
function goalForm(id = null){
  ...
  let h = `
    <div class="card">
      ...
      <label>Nguồn vốn thu xếp sau (VNĐ) - Bán tài sản khác, thừa kế, tài trợ...</label>
...
// GIAO DIỆN KHOẢN VAY ĐỘC LẬP & DÒNG TIỀN
function cash(){
  let t = totals();
  let h = `
    <div class="card">
      <div class="sectionhead"><h2>Khoản vay độc lập</h2><button onclick="loanForm()">+ Thêm khoản vay</button></div>
      <p class="small muted">Nợ không gắn cố định với một BĐS cụ thể.</p>
    </div>
  `;
